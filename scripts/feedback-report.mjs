...
/**
 * Ο ένας reporter που έστειλε feedback χωρίς λογαριασμό ταυτοποιείται από το
 * email του. Δεν γράφεται στον κώδικα: δίνεται μέσω ANON_REPORTER_EMAIL, ώστε
 * να μην υπάρχει προσωπικό δεδομένο τρίτου στο repo.
 */
const ANON_REPORTER = process.env.ANON_REPORTER_EMAIL
  ? `«${process.env.ANON_REPORTER_EMAIL}»`
  : null;
...
const payload = {
  ...
  reporters: perUser.map(([u, n]) => ({ name: u === ANON_REPORTER ? 'χρήστης χωρίς λογαριασμό' : u, n })),
